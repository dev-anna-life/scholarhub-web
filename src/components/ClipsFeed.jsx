...
export default function ClipsFeed() {
  ...
  const [allClips, setAllClips] = useState(SAMPLE_CLIPS)
  const containerRef = useRef(null)

  useEffect(() => {
    async function loadRealUserVideos() {
      try {
        const res = await getPosts(1)
        const postsList = res.data?.posts || res.data || []
        const userVideoPosts = postsList.filter(p => p.video || p.videoUrl)
        if (userVideoPosts.length > 0) {
          const formattedClips = userVideoPosts.map(p => ({
            id: p.id || p._id,
            authorName: typeof p.author === 'object' ? p.author?.name : (p.author || 'Scholar Creator'),
            authorUsername: typeof p.author === 'object' ? (p.author?.username || 'scholar') : 'scholar',
            authorAvatar: typeof p.author === 'object' ? p.author?.avatar : '',
            skillDomain: p.category || 'Scholar Creator',
            title: p.title,
            caption: p.content,
            videoUrl: p.video || p.videoUrl,
            allowDownload: true,
            citationSource: p.citationSource || 'Verified Scholar Video'
          }))
          setAllClips([...formattedClips, ...SAMPLE_CLIPS])
        }
      } catch (e) {
        console.error('Error fetching user video clips:', e)
      }
    }
    loadRealUserVideos()
  }, [])
